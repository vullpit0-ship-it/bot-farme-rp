...
console.log("🔥 BUILD FINAL (FIX usuarios.id MIGRATION + !tabela) 🔥", new Date().toISOString());
...
async function initDB() {
  ...
  if (await tableExists("usuarios")) {
    ...
    if (!info) {
      ...
      // Caso 1: id numérico mas sem identity/default
      ...
      // Caso 2: id não numérico (ex.: text) -> migra para BIGSERIAL
      if (!isNumeric) {
...
client.on("messageCreate", async (message) => {
  try {
    ...
    // ==========================
    // 📊 !tabela (aprovados do dia)
    // Uso: !tabela | !tabela ontem
    // ==========================
    if (lower.startsWith("!tabela")) {
      ...
      const lines = rows.map((r, i) => {
        return `${i + 1}. <@${r.userId}> — 📄 **${r.papel}** | 🌱 **${r.sementes}** | ✅ Total: **${r.total}**`;
      });
