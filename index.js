...
console.log("🔥 BUILD FINAL (FIX usuarios.id MIGRATION + !tabela + !relatorio + !naofarmou) 🔥", new Date().toISOString());
...
// tenta achar role por menção ou por nome digitado
...
client.on("messageCreate", async (message) => {
  try {
    ...
    if (lower.startsWith("!naofarmou")) {
      ...
      const lines = naoFarmaram
        .map((m, i) => `${i + 1}. <@${m.user.id}>`)
        .slice(0, 300); // limite pra não explodir em servidor gigante
